...
db_file = 'amazebot_data.db'  # SQLite database file name
...
def create_table():
    conn = get_connection()
    cursor = conn.cursor()
    cursor.execute('''CREATE TABLE IF NOT EXISTS amazebot_data (
...
def insert_data(display_price, product_name, url, img_src):
    conn = get_connection()
    cursor = conn.cursor()
    sql = "INSERT INTO amazebot_data (price, name, link, img_link) VALUES (?, ?, ?, ?)"
    val = (display_price, product_name, url, img_src)
...
def update_data(display_price, product_name, url, img_src):
    conn = get_connection()
    cursor = conn.cursor()
    cursor.execute("SELECT * FROM amazebot_data WHERE link = ?", (url,))
    existing_record = cursor.fetchone()
    if existing_record:
        print(existing_record)
        cursor.execute("UPDATE amazebot_data SET price = ? WHERE link = ?", (display_price, url))
        conn.commit()
    else:
        insert_data(display_price, product_name, url, img_src)
    cursor.close()
...
def get_data():
    conn = get_connection()
    cursor = conn.cursor()
    cursor.execute("SELECT * FROM amazebot_data")
...
def delete_data(link):
    conn = get_connection()
    cursor = conn.cursor()
    cursor.execute("DELETE FROM amazebot_data WHERE link = ?", (link,))
    conn.commit()
    cursor.close()
    conn.close()
